...
<body>
   ...
   <form action="" method="post">
    ...
    // $total_days=30;
    $query = "SELECT * FROM `attendance` as a JOIN `student` as s ON s.student_id=a.student_id WHERE a.attend_date='$posted_date' AND a.approve_status='approved' AND permission_status='attended'"; 
        ?>
                    <div class="tbl">
                        <?php 
                        // Query
...
if($result = mysqli_query($conn, $query)){
    if(mysqli_num_rows($result) > 0){
        ...
        <div style="color:red; padding:2px;"  >
           <h4 class="text-center">"No attended students now"</h4>
        </div>
      <?php
    }
} else{
